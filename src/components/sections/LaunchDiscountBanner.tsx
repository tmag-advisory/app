...
const LaunchDiscountBanner = ({ variant = "page", className }: LaunchDiscountBannerProps) => {
  ...
  if (variant === "inline") {
    return (
      <div
        role="status"
        className={clsx(
          "flex items-center gap-2 rounded-full border border-emerald-200/70 bg-emerald-50/80 px-3 py-1.5 text-sm font-medium text-emerald-800 shadow-sm",
          className,
        )}
      >
  ...
  return (
    <div
      ...
      className={clsx(
        "relative isolate overflow-hidden rounded-2xl border border-emerald-300/60 bg-gradient-to-r from-emerald-500 via-emerald-600 to-amber-500 px-5 py-4 text-white shadow-lg sm:px-7",
        className,
      )}
    >
