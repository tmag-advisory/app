...
const CreditRequests = () => {
    ...
    const filters = [
        { id: "all", label: "All", count: creditRequests.length },
        { id: "pending", label: "Pending", count: creditRequests.filter((r) => r.status === "pending").length },
        { id: "approved", label: "Approved", count: creditRequests.filter((r) => r.status === "approved").length },
        { id: "rejected", label: "Rejected", count: creditRequests.filter((r) => r.status === "rejected").length },
    ];

    const filtered = filter === "all" ? creditRequests : creditRequests.filter((r) => r.status === filter);

    return (
        <div>
            <DashboardHeader title="Credit requests" />
            ...
            <div className="flex gap-2 mb-6 flex-wrap">
                {filters.map((f) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-semibold transition-colors duration-200 cursor-pointer ${filter === f.id
                            ...
                            {isLoading ? (
                                ...
                                filtered.map((req) => (
                                    <tr key={req.id} className="hover:bg-background-secondary/50 transition-colors duration-150">
                                        ...
                                        <td className="px-6 py-4 text-sm text-body">{req.creditsRequested} credits</td>
                                        <td className="px-6 py-4 text-sm text-muted hidden sm:table-cell">{req.reason || "N/A"}</td>
                                        <td className="px-6 py-4">
